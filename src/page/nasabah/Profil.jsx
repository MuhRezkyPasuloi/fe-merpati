...
const Profil = () => {
  ...
  const fetchUser = async () => {
    try {
      ...
      const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/nasabah/${id}`, {
  ...
  const handleProfileSubmit = async () => {
    try {
      ...
      await axios.put(
        `${import.meta.env.VITE_API_URL}/api/nasabah/${user.id}`,
        formData,
        {
          headers: {
  ...
  const handleLoginSubmit = async () => {
    try {
      const token = Cookies.get('token');
      await axios.put(
        `${import.meta.env.VITE_API_URL}/api/auth/login/${user.login.id}`,
        loginForm,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setEditLogin(false);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="flex items-center mt-1 border rounded-md px-3 py-2">
                ...
                <input type="text" value={`Rp ${user.saldo.toLocaleString('id-ID')}`}
                  readOnly className="w-full outline-none text-sm" />
              </div>
            </div>
            <div>
